...
__global__ void paged_attention_kernel_v5(
    ...
    for (int i = tid; i < num_tokens; i += blockDim.x) {
        ...
        long long k_offset = (long long)physical_block * block_size * num_kv_heads * head_dim
...
void launch_paged_attention_v5(
    torch::Tensor& out,
    torch::Tensor& query,
...
) 
{
    ...
    paged_attention_kernel_v5<<<grid, block>>>(
        out.data_ptr<float>(),
