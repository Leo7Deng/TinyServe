...
### `attention_v7.cu`
After profiling attention_v6 with Nsight Compute, I found two main concerns. First, the Memory Workload Analysis section reported inefficient local loads and stores, which usually means per-thread data is spilling out of registers into local memory. One of the largest per-thread arrays in V6 was `float local_acc[MAX_HEAD_DIM]`, so V7 stores the accumulator as `float4 local_acc[16]` to better match the vectorized K/V loads and make register allocation easier for the compiler. Second, Nsight Compute showed that when a warp accessed shared memory, many threads were hitting the same shared memory bank at the same time. Following `shared_acc`, the issue came from neighboring threads accessing `shared_acc[thread][dim]` with a power-of-two row stride. V7 changes the layout to `shared_acc[dim][thread]` with a `+1` padded column so neighboring threads are spread across different banks during the reduction.
